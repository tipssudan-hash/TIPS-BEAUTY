...
import type { CollectionInput, Product } from '../../types';
import {
    activateCollection, fetchAdminProducts, fetchCollection, fetchCollectionProductIds,
...
const emptyCollection: CollectionInput = {
    slug: '', name_ar: '', description_ar: null, icon: 'auto-awesome', display_order: 100, is_active: true,
};
...
export const CollectionFormPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetchAdminProducts(),
            id ? fetchCollection(id) : Promise.resolve(null),
            id ? fetchCollectionProductIds(id) : Promise.resolve<string[]>([]),
        ])
            .then(([products, collection, productIds]) => {
                if (cancelled) return;
                setCatalogue(products);
                if (id) {
                    if (!collection) { navigate('/collections'); return; }
                    const { id: _id, ...input } = collection;
                    void _id;
                    setForm(input);
                    setSelectedIds(productIds);
                }
            })
            .catch((err) => { if (!cancelled) setError(errorMessage(err, 'تعذر تحميل بيانات المجموعة.')); })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setError(null);
        if (!form.name_ar.trim()) { setError('اسم المجموعة مطلوب.'); return; }
        const slugError = validateCollectionSlug(form.slug);
        if (slugError) { setError(slugError); return; }

        setSaving(true);
        try {
            const collectionId = await saveCollection(id ?? null, form);
            await setCollectionProducts(collectionId, selectedIds);
            if (!id && form.is_active) await activateCollection(collectionId);
            navigate('/collections');
        } catch (err) {
            setError(errorMessage(err, 'فشل حفظ المجموعة.'));
        } finally {
            setSaving(false);
        }
    };

    if (fetching) return <Spinner label="جاري تحميل بيانات المجموعة..." />;
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-5 rounded-3xl border border-slate-100 shadow-sm sticky top-4 z-30">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-black text-slate-900">{isEditMode ? 'تعديل المجموعة' : 'مجموعة جديدة'}</h1>
                </div>
            ...
            <form id="collection-form" onSubmit={handleSubmit} className="space-y-8">
                <Card className="p-8 space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 items-end">
                        <Field label="ترتيب العرض" hint="الأصغر يظهر أولاً بين المجموعات">
                ...
                <Card className="p-8 space-y-6">
                    <div>
                        <h3 className="text-lg font-black text-slate-900">منتجات المجموعة</h3>
                        <p className="text-xs text-slate-400 font-bold mt-1">منتج غير متوفر حالياً لا يظهر للعميل حتى يعود للمخزون، لكن يبقى ضمن المجموعة.</p>
                    </div>
                            ...
                            {selectedProducts.map((p, idx) => (
                                <div key={p.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        ...
                                        <button type="button" onClick={() => removeProduct(p.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg" aria-label="إزالة من المجموعة"><X className="w-4 h-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
