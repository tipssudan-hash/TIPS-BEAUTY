...
export const BannerCarousel: React.FC<BannerCarouselProps> = ({ banners, onSelect, intervalMs = 6000 }) => {
    ...
    return (
        <section aria-roledescription="carousel" aria-label="العروض والإعلانات" className="relative mb-8 max-w-4xl mx-auto px-4">
            <div
                ref={track}
                    ...
                    return (
                        <div key={banner.id} className="w-full shrink-0 snap-start" role="group" aria-roledescription="slide" aria-label={`${index + 1} من ${banners.length}`}>
                            <button
                                ...
                                className="relative block w-full overflow-hidden rounded-2xl bg-brand-blue-soft text-right shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue disabled:cursor-default"
